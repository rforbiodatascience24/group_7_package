...
#'
#' @return A barplot of the occurence of each unique amino acid.
#' @export
#'
#' @examples sequence <- paste0(sample(c("A", "C", "D", "E", "F", "G", "H",
#'                                       "I", "K", "L", "M", "N", "P", "Q",
#'                                       "R", "S", "T", "V", "W", "Y"),
...
plot_occurrence <- function(amino_acid_sequence){
  ...
  counts <- sapply(unique_amino_acids,
                   function(amino_acid)
                     stringr::str_count(string = amino_acid_sequence,
                                        pattern =  amino_acid)) |>
    as.data.frame()
  ...
  amino_acid_occurrence <- counts |>
    ggplot2::ggplot(ggplot2::aes(x = amino_acid_sequence,
                                 y = Counts,
                                 fill = amino_acid_sequence)) +
    ggplot2::geom_col() +
    ggplot2::theme_bw() +
    ggplot2::theme(legend.position = "none",
                   axis.title.x = "Amino acid")
