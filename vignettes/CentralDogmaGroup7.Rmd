...
Function three, `codon_extractor()` converts a nucleic acid sequence into it's respective codons. It takes a string input (e.g. 'GCCATATAG') and returns a new string containing the codons of the input sequence (e.g. 'GCC ATA TAG'). Function three, `codon_extractor()` converts a nucleic acid sequence into it's respective codons. It takes a string input and returns a new string containing the codons of the input sequence.

```{r}
#Example
codon_extractor('ATTGATTAG')
```

### Function 4

The `translate_rna()` function is part of the central dogma simulation, designed to take a vector of RNA codons and convert it into a protein sequence. It maps each codon to its corresponding amino acid using a codon table, concatenates the amino acids, and returns the complete protein sequence as output.
